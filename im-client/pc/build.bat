...
echo [3/7] 准备便携 Node 运行时（bundled\ 内无 zip 则下载缓存，仅首次 34.5MB）...
...
echo [4/7] 准备 uv 工具链（bundled\ 内无 zip 则下载缓存，仅首次约 17MB）...
...
echo [5/7] JS 混淆打包（TRAE CN 同款：esbuild 压缩+变量名混淆，输出 bundled\web-obfuscated）...
...
rem 阶段一百三十六：原[6/8] robocopy 明文快照步骤已移除——加密快照 web-snapshot.enc 与 secure-key.js
rem 现由 obfuscate.js 直接生成并经 package.json files 规则打入 asar，明文 web-snapshot 无消费方。
echo [6/7] 打包 win-unpacked...
